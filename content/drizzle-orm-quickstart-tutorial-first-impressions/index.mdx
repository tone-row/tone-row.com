...
The last thing we need to confirm is that we truly have a typesafe-client. Let's add a script to `src/demo.ts` that will real all of the users from the database:
...
The schema is expressive and seems ready to use in production, but type-safety seems to come with more manual work. In terms of migrations, I wish it was possible in one step with the Drizzle CLI instead of two, using two packages.

I'm interested to see how Drizzle's adoption grows. If it has smooth integrations with things like next-auth and trpc in future that could further tip the scales in its favor.
